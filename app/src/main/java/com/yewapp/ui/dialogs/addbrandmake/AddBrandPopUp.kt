...
class AddBrandPopUp(
    val sportList: MutableList<Brand>,
    val listener: (Brand) -> Unit
) {
...
//        val width = ConstraintLayout.LayoutParams.MATCH_PARENT
        ...
        recyclerView.adapter = AddBrandPopUpAdapter(sportList) {
            popupWindow.dismiss()
            listener(it)
        }
    ...
    class Builder() {
        var sportList: List<Brand> = mutableListOf()
        var itemListener: (Brand) -> Unit = {}

        fun setListener(listener: (Brand) -> Unit): Builder {
            this.itemListener = listener
            return this
        }

        fun addList(list: List<Brand>): Builder {
            this.sportList = list
            return this
        }

        fun build(): AddBrandPopUp {
            return AddBrandPopUp(sportList as MutableList<Brand>, itemListener)
        }
    }
}