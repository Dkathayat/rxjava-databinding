...
import com.yewapp.ui.dialogs.challengepopupdialogs.adapter.ChallengePopUpModelAdapter

class ChallengeModelPopUp(
...
//        val width = ConstraintLayout.LayoutParams.MATCH_PARENT
        ...
        recyclerView.adapter = ChallengePopUpModelAdapter(modelList) {
            popupWindow.dismiss()
            listener(it)
        }
    ...
    class Builder() {
        var modelList: List<Model> = mutableListOf()
        ...
        fun build(): ChallengeModelPopUp {
            return ChallengeModelPopUp(modelList as MutableList<Model>, itemListener)
        }
    }
}