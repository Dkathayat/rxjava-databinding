...
class AddModelViewModel(dataManager: DataManager, schedulerProvider: SchedulerProvider) :
    BaseViewModel<AddModelNavigator>(dataManager, schedulerProvider) {
    ...
    var addWheelSizeHashMap = HashMap<View, WheelSize>()
    ...
    private fun getSavedSportsEquipmentsSuccess(response: GetSavedModelEquipmentResponse) {
        ...
        if (response.equipmentData.isNotEmpty()) {
            ...
            for (i in 0 until response.equipmentData.size) {
                if (response.equipmentData[i].type.equals("model", ignoreCase = true))
