...
import com.yewapp.databinding.FavoritesFragmentBinding
...
class FavoritesRoutesFragment(override val layoutId: Int = R.layout.favorites_fragment) :
    BaseFragment<FavoritesRoutesNavigator, FavoritesRoutesViewModel, FavoritesFragmentBinding>(),
    FavoritesRoutesNavigator {

    private lateinit var popularRoutesAdapter: PopularRoutesAdapter

    companion object {
        fun newInstance(challengeModel: ChallengeModel): FavoritesRoutesFragment {
            //can not send without bundle else will give null pointer error
            val fragment = FavoritesRoutesFragment()
            val args = Bundle()
    ...
    override fun onResume() {
        super.onResume()
        viewModel?.getFavoritesRoutesList()
    }

    override fun onViewModelCreated(viewModel: FavoritesRoutesViewModel) {
        viewModel.setNavigator(this)
    }

    override fun onViewBound(viewDataBinding: FavoritesFragmentBinding) {
        adapterInitialize()
    }
    ...
    override fun onCreateView(
        ...
        return bind(FavoritesRoutesViewModel::class.java, inflater, container)
    }
    ...
    private fun addObserver() {
        ...
        viewModel.favoritesRouteList.observe(this, Observer {
    ...
    private fun adapterInitialize() {
                ...
                override fun onClickItem(item: Route) {
                    val extra = ChallengeExtras.challengeExtras {
                        challengeData = ChallengeModel(
                            ...
                            viewModel?.challengeModel?.stateId?:0, arrayListOf(), arrayListOf(),
                            "", "", "", "", "",
                            "", "", "", "", "",
        ...
        viewDataBinding.favoritesRecycler.adapter = popularRoutesAdapter
